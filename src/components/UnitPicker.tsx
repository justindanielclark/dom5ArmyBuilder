...
function UnitPicker({ units, chosenUnit, handleChooseUnit }: Props) {
  const [search, setSearch] = useState("");
  const filteredUnits = units.filter((unit) =>
    unit.name.toLowerCase().includes(search.toLowerCase())
  );
  return (
    <section className="py-3 px-5 flex-1">
      <div>
        <h1 className="text-2xl font-bold mb-2">Unit Search:</h1>
        <div>
          <input
            value={search}
            type="text"
                ...
                .map((unit) => {
                  return (
                    <li key={unit.id} className="flex flex-row w-full relative">
                      <input
                        checked={
                          chosenUnit && chosenUnit.id === unit.id ? true : false
                        }
                        className="mr-2"
                        type="checkbox"
                        onChange={(e) => {
                      ...
                      <div className="hidden hover:flex absolute z-50 bottom-4 right-2 rounded-lg translate-x-full translate-y-full peer-hover:flex flex-col bg-slate-800 border-white border-2 text-xs gap-2">
                                  ...
                                  {unitMagicKeys
                                    ...
                                    .map((validPath) => (
                                      ...
                                      >
                                        <img
                                          className="h-4"
                                          src={magicPathIcons[validPath.path]}
                                        />
                                        <p>{validPath.value}</p>
                                      </div>
                                    ))}
